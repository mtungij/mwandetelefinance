    ...
    //    exit();
     ...
     return true;
      ...
      public function disburse_loan(){
        ...
        $dis=$this->queries->get_DisbarsedBlanch_data($blanch_id);
        ...
        $total_interest_loan = $this->queries->get_sum_loanDisburse_interest_blanch($blanch_id);
            ...
            // echo "</pre>";
            //     exit();
        $this->load->view('oficer/disburse_loan',['disburse'=>$disburse, 'dis'=>$dis,'total_loanDis'=>$total_loanDis,'total_interest_loan'=>$total_interest_loan]);
    }

    public function view_aggrement($customer_id,$loan_id)
    {
      ...
      $this->load->model('queries');
		        ...
		        $blanch_data = $this->queries->get_blanchData($blanch_id);
		        $empl_data = $this->queries->get_employee_data($empl_id);
				$customer_profile = $this->queries->get_customer_profile($customer_id);
        $collateral = $this->queries->get_colateral_data($loan_id);
				$blanch = $this->queries->get_blanch($comp_id);
				$mdhamini  = $this->queries->get_guarantors_data($customer_id);
        $loan_customer = $this->queries->get_loan_customer($customer_id);
        ...
        $mpdf->SetWatermarkText($company_data->comp_name);
        $mpdf->showWatermarkText = true;
        $date = date("Y-m-d");
        $mpdf->WriteHTML($html);
        $output = 'mkataba_wa_maombi.pdf ' . $date.'.pdf';
